...
- **Where it shows up:** `xzregh/108D50_payload_stream_decrypt_and_append_chunk.c:57-77` and `xzregh/1094A0_rsa_backdoor_command_dispatch.c:120-318` still reference `payload->field0_0x0` / `encrypted_payload.field0_0x0` with raw `_N_M_` slices for the stride/index/bias nonce, encrypted-body length, and command flag bytes.
...
- **Where it shows up:** `xzregh/107A20_sshd_find_forged_modulus_sshbuf.c:41-63`, `xzregh/107950_sshbuf_extract_ptr_and_len.c:31-63`, and `xzregh/1094A0_rsa_backdoor_command_dispatch.c:250-326` keep poking `*(byte *)((long)&(ctx->sshd_offsets).field0_0x0 + N)` to recover the pkex slot, sshbuf data/size qword indexes, etc.
...
- **Where it shows up:** Instruction scanners such as `xzregh/104AE0_find_l_audit_any_plt_mask_and_slot.c:76-192`, `xzregh/104EE0_find_l_audit_any_plt_mask_via_symbind_alt.c:65-145`, and `xzregh/103680_sshd_find_sensitive_data_base_via_xcalloc.c:74` read `insn_ctx._40_4_`, `insn_ctx.prefix._15_1_`, etc. to decode ModRM/REX bits.
...
- **Where it shows up:** `xzregh/104EE0_find_l_audit_any_plt_mask_via_symbind_alt.c:65-145` manipulates `mask_register_bitmap._0_3_`, `output_register_bitmap._0_2_`, `search_ctx.offset_to_match._0_4_`, etc., suggesting these locals are really small structs/bitfields the matcher expects.
...
- **Status (2025-12-16):** Completed – introduced `instruction_register_bitmap_t` + `instruction_search_offset_t` in `metadata/xzre_types.json`, retagged `instruction_search_ctx_t` to use them, and refreshed the project so `xzregh/104EE0_find_l_audit_any_plt_mask_via_symbind_alt.c`/`xzregh/104AE0_find_l_audit_any_plt_mask_and_slot.c` now export `offset_to_match.dwords.offset` plus `bitmap.fields.{allowed_regs,reg_index}` instead of `_0_3_`/`_0_4_` slices.
