...
- Private key: every command blob is signed with an attacker-held Ed448 private key; the implant only carries the raw Ed448 public key (stored inside the 57-byte `secret_data` buffer and unwrapped via a two-pass ChaCha decrypt in `secret_data_get_decrypted`). The first decrypt peels a seed that becomes the runtime ChaCha key, the second decrypt reveals the Ed448 key plus the symmetric payload material. Without the Ed448 private key, signatures fail and the hooks fall back to the original OpenSSL routines.
...
## Command Types & Flags
The decrypted payload body exposes a `cmd_arguments_t` block so the attacker can toggle exit/log/PAM behaviour without crafting new opcodes. `run_backdoor_commands` stores the parsed block inside `key_ctx_t` and mirrors it into the `monitor_data_t` that eventually reaches `sshd_proxy_elevate`.

- `cmd_type`: lower values drive in-place state updates (sshd variable patches, payload buffering, continuation fragments), while `cmd_type == 3` asks `sshd_proxy_elevate` to forge a `MONITOR_REQ_KEYALLOWED` exchange (building temporary RSA/BIGNUM objects with the attacker modulus/exponent).
...
- `request_flags`: the low five bits either override the MONITOR_REQ id or encode an additional socket number, bit5 requests that `sshd_proxy_elevate` pull an sshbuf via `sshd_get_sshbuf`, and bit6 turns the trailing payload bytes into packed offset fields.
...
The runtime keeps two helper structs in sync while processing these flags. `run_backdoor_commands_data_t` owns the staging buffers (socket receive queues, host-key digests, monitor union, Ed448 key cache), while `key_ctx_t` carries the decrypted payload, modulus/exponent pointers, ChaCha nonce/IV snapshots, and the unwrapped Ed448 key so subsequent chunks can be verified without re-reading `secret_data`.
...
- Every ELF walker, relocation scanner, and sshd heuristic starts by calling one of the `secret_data_append_*` helpers. `secret_data_append_from_call_site`, `_from_address`, `_from_code`, and `_from_instruction` feed a 0x1C8-bit cursor (`secret_data_shift_cursor_t`) inside `global_context_t->secret_data`, while `secret_data_append_items` ensures each `secret_data_item_t` descriptor only runs once (gated by the `shift_operation_flags` array). If any appender fails, the caller aborts instead of mutating GOT slots or monitor state off-record.
- `run_backdoor_commands` refuses to decrypt payloads unless `ctx->secret_bits_filled == 0x1C8`, `ctx->sshd_log_ctx` is non-null, and the loader recorded every expected attestation. That ties command execution to the success of the discovery/telemetry path and prevents partially initialised hosts from running attacker opcodes.
- The `encrypted_secret_data[57]` buffer inside `global_context_t` doubles as storage for the bit-shift log and for the encrypted Ed448/ChaCha material. `secret_data_get_decrypted` performs a two-step ChaCha decrypt: a built-in key unwraps a 0x30-byte seed, the seed becomes the runtime ChaCha key, and a second decrypt reveals the Ed448 public key plus the ChaCha key/IV that decrypt payload bodies.
- String lookups never embed plaintext. `get_string_id` walks the packed trie baked into `string_action_data`/`string_mask_data` using `count_bits` to choose each branch; every call reports itself via `secret_data_append_from_address` so the attestation log mirrors successful string scrapes. The resulting encoded IDs let the loader find sshd banner strings, log format specifiers, and monitor helpers without shipping readable literals.
