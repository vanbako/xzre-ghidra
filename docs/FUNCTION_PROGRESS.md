...
| `1094A0_run_backdoor_commands` | `crypto_cmd` | 3 | 2025-11-26 (CC3 deep dive) – retagged the RSA modulus/command-flag locals (`rsa_modulus_bytes`, `payload_buffer_cursor`, `socket_probe_header`, decrypted/encrypted payload buffers), added inline anchors for the opcode derivation, caller-uid capture, sshd_offsets repack, and mm_answer_keyallowed hook, and reran the refresh so the dispatcher now mirrors the metadata. |
