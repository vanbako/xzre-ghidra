...
| `1094A0_run_backdoor_commands` | `crypto_cmd` | 2 | 2025-11-25 (CC3 revisit) – renamed the payload body locals (`payload_plaintext_size`, `payload_plaintext`, `hostkey_idx`, `bytes_read`, `pselect_result`), added inline anchors for the PAM disable bit, system-exec opcode, kill-switch block, and continuation streaming logic, and reran the refresh so the regenerated C mirrors the metadata. |
