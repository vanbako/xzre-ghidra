...
| `BIGNUM` | 0 |  |
| `BN_CTX` | 0 |  |
| `cmd_arguments_t` | 3 | Payload/Crypto review – renamed the three flag bytes to control/monitor/request flags, annotated their bit semantics (log hook, PAM, socket/payload sourcing), and documented how payload_hint doubles as a length vs. sshd_offsets overlay (2025-11-21); added `CmdControlFlags_t`/`CMD_CTRL_*` to name control_flags bits across SR4 hooks (2025-12-23); added `CmdMonitorFlags`/`CmdRequestFlags` plus socket-field constants to name monitor/request low-bit selectors in SR4 dispatch (2025-12-23). |
| `cmd_flags_tail_t` | 1 | Added cmd_flags tail bitfield overlays (`offsets_v0`/`offsets_v1`) to name packed sshd_offsets tail fields and gating bits in rsa_backdoor_command_dispatch (2025-12-26). |
...
| `instruction_register_bitmap_t` | 3 | Added the 4-byte `{allowed_regs, reg_index}` union used by the audit instruction matcher so exports stop relying on `_0_3_` slices (2025-12-16); added a `high_word` overlay for the reg_index/reserved halfword so scanners stop using `_2_2_` slices (2025-12-21); typed `allowed_regs` as `X86RegisterMask_t` and documented the register-mask constants for audit register filters (2025-12-25). |
...
| `La_x86_64_regs` | 0 |  |
| `La_x86_64_retval` | 0 |  |
| `ldso_ctx_t` | 2 | Named `libcrypto_basename_buf` and documented how stage one uses it to forge `link_map::l_name` during the ld.so audit install (2025-12-18); typed the `l_audit_any_plt` flag pointer/mask as `LinkMapAuditFlags_t` (2025-12-25). |
...
| `secret_data_item_t` | 2 | Renamed entries to anchor_pc/bit_cursor/operation_slot/bits_to_shift/ordinal and annotated how each controls the secret-data attestation flow (2025-11-21); retyped the SR2 bootstrap batch as `secret_data_item_t[4]` so packed words expand into per-field assignments (2025-12-23). |
