...
- Session `RENAMES_CC4_RECHECK`: revisited the crypto_cmd CC4 secret-data helpers and tightened rename suggestions in `metadata/backdoor_function_renames.json`: `secret_data_append_from_instruction`→`secret_data_append_opcode_bit`, `secret_data_append_from_code`→`secret_data_append_code_bits`, `secret_data_append_singleton`→`secret_data_append_singleton_bits`, and `secret_data_append_item`→`secret_data_append_item_if_enabled` (kept `get_string_id` as `encoded_string_id_lookup`). Next: continue with CC5 or adjust naming length if desired.
- Session `RENAMES_CC3_RECHECK`: revisited the crypto_cmd CC3 RSA hook/dispatcher helpers and tightened rename suggestions in `metadata/backdoor_function_renames.json`: `run_backdoor_commands`→`rsa_backdoor_command_dispatch`, `hook_RSA_public_decrypt`→`rsa_public_decrypt_backdoor_shim`, `hook_EVP_PKEY_set1_RSA`→`evp_pkey_set1_rsa_backdoor_shim`, and `hook_RSA_get0_key`→`rsa_get0_key_backdoor_shim` (kept `count_bits` as `popcount_u64`). Next: continue with CC4/CC5 rename passes or adjust naming length if desired.
- Session `RENAMES_CC2_RECHECK`: revisited the crypto_cmd CC2 helpers and tightened rename suggestions in `metadata/backdoor_function_renames.json`: `verify_signature`→`verify_ed448_signed_payload`, `sshbuf_bignum_is_negative`→`sshbuf_is_negative_mpint`, `sshbuf_extract`→`sshbuf_extract_ptr_and_len`, and `secret_data_get_decrypted`→`secret_data_decrypt_with_embedded_seed` (kept `sshd_get_sshbuf` as `sshd_find_forged_modulus_sshbuf`). Next: continue with CC3 or revisit any identity names you want shorter.
- Session `RENAMES_CC1_RECHECK`: revisited the crypto_cmd CC1 primitives and tightened the rename suggestions in `metadata/backdoor_function_renames.json`: `dsa_key_hash`→`dsa_pubkey_sha256_fingerprint`, `rsa_key_hash`→`rsa_pubkey_sha256_fingerprint`, `bignum_serialize`→`bignum_mpint_serialize`, `chacha_decrypt`→`chacha20_decrypt`, and `sha256`→`sha256_digest`. Next: continue the `crypto_cmd` rename pass for CC2 or move to other batches.
- Session `RENAMES_LR6_CFUNCS`: refined the libc-stand-in names in `metadata/backdoor_function_renames.json`: `c_memmove`→`memmove_overlap_safe`, `c_strlen`→`strlen_unbounded`, and `c_strnlen`→`strnlen_bounded`. Next: continue with the next batch (e.g., `crypto_cmd`) or revisit other identity mappings if desired.
- Session `RENAMES_LR6_RECHECK`: rechecked the LR6 stdlib/IO helpers (`c_strlen`, `c_strnlen`, `fd_read`, `fd_write`, `contains_null_pointers`) and kept the existing rename suggestions in `metadata/backdoor_function_renames.json` unchanged. Next: proceed to the next batch (e.g., `crypto_cmd`) or revisit any identity mappings if desired.
- Session `RENAMES_LR5_RECHECK`: rechecked the LR5 cpuid-entry helpers (`backdoor_entry`, `_get_cpuid_modified`, `_cpuid_gcc`, `count_pointers`, `xzre_globals`) and kept the existing rename suggestions in `metadata/backdoor_function_renames.json` unchanged after review. Next: proceed to the next batch (e.g., `crypto_cmd`).
- Session `RENAMES_LR5`: finalized the loader_rt rename pass by adding the data-symbol mapping `xzre_globals`→`backdoor_hooks_data_blob` in `metadata/backdoor_function_renames.json` (left `c_strlen`, `c_strnlen`, and `_cpuid_gcc` unchanged). Next: move on to the next batch outside loader_rt (e.g., `crypto_cmd`).
- Session `RENAMES_LR4`: reviewed the remaining loader_rt helpers and updated `metadata/backdoor_function_renames.json` with rename suggestions: `fd_read`→`fd_read_full`, `fd_write`→`fd_write_full`, `contains_null_pointers`→`pointer_array_has_null`, and `count_pointers`→`count_null_terminated_ptrs` (kept `c_strlen`, `c_strnlen`, and `_cpuid_gcc` unchanged; still omitting `xzre_globals` from the rename map). Next: decide whether to keep the identity names for the libc/string helpers or refine them further, then move on to the next batch outside loader_rt.
- Session `RENAMES_LR3`: reviewed loader_rt LR3 cpuid resolver + setup helpers and updated `metadata/backdoor_function_renames.json` with rename suggestions: `backdoor_setup`→`backdoor_install_runtime_hooks`, `backdoor_init_stage2`→`cpuid_ifunc_stage2_install_hooks`, `backdoor_init`→`cpuid_ifunc_patch_got_for_stage2`, `backdoor_entry`→`cpuid_ifunc_resolver_entry`, and `_get_cpuid_modified`→`get_cpuid_with_ifunc_bootstrap`. Next: continue with loader_rt LR4 (remaining helper utilities like `fd_read`/`fd_write`/`contains_null_pointers`/`count_pointers`/`_cpuid_gcc`, and decide whether to add `xzre_globals` to the rename map).
- Session `RENAMES_LR2`: reviewed loader_rt LR2 link-map/audit/shared-lib helpers and updated `metadata/backdoor_function_renames.json` with rename suggestions: `find_link_map_l_name`→`find_link_map_l_name_offsets`, `find_dl_naudit`→`find_dl_naudit_slot`, `find_link_map_l_audit_any_plt`→`find_l_audit_any_plt_mask_via_symbind_alt`, `find_link_map_l_audit_any_plt_bitmask`→`find_l_audit_any_plt_mask_and_slot`, `find_dl_audit_offsets`→`resolve_ldso_audit_offsets`, `process_shared_libraries`→`scan_shared_libraries_via_r_debug`, `process_shared_libraries_map`→`scan_link_map_and_init_shared_libs`, and `resolve_libc_imports`→`resolve_libc_read_errno_imports`. Next: continue with loader_rt LR3 (backdoor setup/stage2 hooks + remaining runtime helpers).
- Session `RENAMES_LR1`: reviewed loader_rt LR1 init/import helpers and updated `metadata/backdoor_function_renames.json` with rename suggestions: `init_hooks_ctx`→`hooks_ctx_init_or_wait_for_shared_globals`, `init_shared_globals`→`init_backdoor_shared_globals`, `init_imported_funcs`→`libcrypto_imports_ready_or_install_bootstrap`, `validate_log_handler_pointers`→`sshd_validate_log_handler_slots`, `update_cpuid_got_index`→`cache_cpuid_gotplt_slot_index`, `get_tls_get_addr_random_symbol_got_offset`→`seed_got_ctx_for_tls_get_addr_parse`, and `update_got_offset`→`cache_got_base_offset_from_cpuid_anchor` (kept `init_ldso_ctx`/`update_got_address` unchanged). Next: continue with loader_rt LR2 (link_map/audit/GOT helpers).
- Session `RENAMES_SR5`: final consistency pass over the SR5 payload/log hooks and updated `metadata/backdoor_function_renames.json` with tighter names: `check_backdoor_state`→`payload_stream_validate_or_poison`, `decrypt_payload_message`→`payload_stream_decrypt_and_append_chunk`, `mm_log_handler_hook`→`mm_log_handler_hide_auth_success_hook`, and `sshd_patch_variables`→`sshd_patch_permitrootlogin_usepam_and_hook_authpassword`. Next: continue with the remaining `loader_rt` rename backlog.
- Session `RENAMES_SR4`: revisited the SR4 sshd monitor/payload hooks and updated `metadata/backdoor_function_renames.json` with rename suggestions: `sshd_proxy_elevate`→`sshd_monitor_cmd_dispatch`, `sshd_configure_log_hook`→`sshd_install_mm_log_handler_hook`, `mm_answer_keyallowed_hook`→`mm_answer_keyallowed_payload_dispatch_hook`, `mm_answer_keyverify_hook`→`mm_answer_keyverify_send_staged_reply_hook`, `mm_answer_authpassword_hook`→`mm_answer_authpassword_send_reply_hook`, and `mm_log_handler_hook`→`mm_log_handler_rewrite_auth_success_hook`. Next: continue with `sshd_recon` SR5 (remaining monitor hooks + dispatcher glue) or pivot to `loader_rt` renames.
- Session `RENAMES_SR3`: revisited the SR3 sshd recon helpers (monitor slot voting, libcrypto import priming, log wrapper, fd probing, and authpassword hook install) and updated `metadata/backdoor_function_renames.json` with rename suggestions: `sshd_find_monitor_struct`→`sshd_find_monitor_ptr_slot`, `sshd_find_sensitive_data`→`sshd_recon_bootstrap_sensitive_data`, `sshd_log`→`sshd_log_via_sshlogv`, `sshd_get_usable_socket`→`sshd_find_socket_fd_by_shutdown_probe`, `sshd_get_client_socket`→`sshd_get_monitor_comm_fd`, and `sshd_patch_variables`→`sshd_patch_permit_rootlogin_pam_and_install_authpassword_hook`. Next: continue with `sshd_recon` SR4 (log shim usage + patch variables in the hooks) or pivot to the remaining `loader_rt` rename backlog.
- Session `RENAMES_SR2`: revisited the SR2 sshd recon helpers (argv/env validation + scoring aggregation) and updated `metadata/backdoor_function_renames.json` with rename suggestions: `check_argument`→`argv_dash_option_contains_lowercase_d`, `process_is_sshd`→`sshd_validate_stack_argv_envp_layout`, `sshd_get_sensitive_data_score_in_main`→`sshd_score_sensitive_data_candidate_in_main`, `sshd_get_sensitive_data_score_in_demote_sensitive_data`→`sshd_score_sensitive_data_candidate_in_demote_sensitive_data`, and `sshd_get_sensitive_data_score`→`sshd_score_sensitive_data_candidate`. Next: continue with `sshd_recon` SR3 (monitor struct discovery + recon orchestrator).
- Session `RENAMES_SR1`: revisited `sshd_recon` SR1 helpers and updated `metadata/backdoor_function_renames.json` with tighter rename suggestions: `sshd_find_main`→`sshd_find_main_from_entry_stub`, `sshd_find_monitor_field_addr_in_function`→`sshd_find_monitor_field_slot_via_mm_request_send`, `sshd_get_sensitive_data_address_via_krb5ccname`→`sshd_find_sensitive_data_base_via_krb5ccname`, `sshd_get_sensitive_data_address_via_xcalloc`→`sshd_find_sensitive_data_base_via_xcalloc`, and `sshd_get_sensitive_data_score_in_do_child`→`sshd_score_sensitive_data_candidate_in_do_child`. Next: continue with `sshd_recon` SR2 (argument/env validation and score aggregation helpers).
- Session `RENAMES_EL6`: adjusted the misleading `elf_get_data_segment` rename suggestion now that its behavior is clearly “BSS/padding tail window” rather than the full segment: `elf_get_data_segment`→`elf_get_writable_tail_span` in `metadata/backdoor_function_renames.json`. Next: continue the rename pass for `loader_rt` (GOT/link_map/audit helpers).
- Session `RENAMES_EL5`: wrapped up the `elf_mem` rename pass by tightening two remaining generic names in `metadata/backdoor_function_renames.json`: `elf_parse`→`elf_info_parse` and `main_elf_parse`→`main_elf_resolve_stack_end_if_sshd`. Next: start the same rename pass for `loader_rt` (GOT/link_map/audit helpers).
- Session `RENAMES_EL4`: refined remaining `elf_mem` rename suggestions and updated `metadata/backdoor_function_renames.json` with more specific names: `elf_symbol_get`→`elf_gnu_hash_lookup_symbol`, `elf_symbol_get_addr`→`elf_gnu_hash_lookup_symbol_addr`, `elf_find_rela_reloc`→`elf_rela_find_relative_slot`, `elf_find_relr_reloc`→`elf_relr_find_relative_slot`, and `is_range_mapped`→`is_range_mapped_via_pselect`. Next: if you’re happy with the `elf_mem` set, we can start the same rename pass for the remaining `loader_rt` helpers.
- Session `RENAMES_EL3`: continued the rename pass for `elf_mem` EL3 helpers and updated `metadata/backdoor_function_renames.json` with rename suggestions for the allocator/TLS plumbing: `fake_lzma_alloc`→`fake_lzma_alloc_resolve_symbol`, `fake_lzma_free`→`fake_lzma_free_noop`, `j_tls_get_addr`→`tls_get_addr_trampoline`, `get_lzma_allocator_address`→`get_fake_lzma_allocator_blob`, `get_elf_functions_address`→`get_elf_functions_table`, and `get_lzma_allocator`→`get_fake_lzma_allocator` (added identity entries for `c_memmove`, `is_range_mapped`, `init_elf_entry_ctx`, `main_elf_parse`). Next: decide whether to keep `is_range_mapped` as-is or rename it to reflect the `pselect(sigmask)` probe trick, and then continue with the remaining `elf_mem`/loader runtime helpers (TLS/GOT address utilities).
- Session `RENAMES_EL2`: continued the rename pass for `elf_mem` EL2 helpers and updated `metadata/backdoor_function_renames.json` with additions/renames: `elf_get_reloc_symbol`→`elf_find_import_reloc_slot`, `elf_get_plt_symbol`→`elf_find_plt_reloc_slot`, `elf_get_got_symbol`→`elf_find_got_reloc_slot`, `elf_get_code_segment`→`elf_get_text_segment`, `elf_find_string`→`elf_find_encoded_string_in_rodata`, and `elf_contains_vaddr_relro`→`elf_vaddr_range_in_relro_if_required` (plus identity entries for the other EL2 helpers). Next: continue with the remaining `elf_mem` helpers (allocator shims, `is_range_mapped`, TLS/GOT address helpers) and decide whether `elf_find_{rela,relr}_reloc` should be renamed to explicitly call out that they only accept RELATIVE relocations.
- Session `RENAMES_EL1`: started renaming pass for `elf_mem` EL1 containment/parser helpers and updated `metadata/backdoor_function_renames.json` with suggestions: `elf_contains_vaddr_impl`→`elf_vaddr_range_has_pflags_impl`, `elf_contains_vaddr`→`elf_vaddr_range_has_pflags`, and `is_gnu_relro`→`is_pt_gnu_relro` (kept `elf_parse` as-is). Next: continue EL2 (symbol/reloc helpers) and decide whether to rename `elf_contains_vaddr_relro` to match the new `elf_vaddr_range_*` naming.
- Session `RENAMES_OP4`: revisited `opco_patt` OP4 string-xref table builders and updated `metadata/backdoor_function_renames.json` with rename suggestions: `elf_find_string_reference`→`elf_find_encoded_string_xref_site` and `elf_find_string_references`→`elf_build_string_xref_table`. Next: if we keep expanding this mapping beyond backdoor-only code, consider renaming the file (or splitting it by batch) before the automated rename script lands.
- Session `RENAMES_OP3`: revisited `opco_patt` OP3 memory-operand sweep helpers and updated `metadata/backdoor_function_renames.json` with rename suggestions to reflect the actual RIP-relative patterns: `find_instruction_with_mem_operand_ex`→`find_riprel_opcode_memref_ex`, `find_instruction_with_mem_operand`→`find_riprel_ptr_lea_or_mov_load`, `find_add_instruction_with_mem_operand`→`find_riprel_grp1_imm8_memref` (covers all GRP1 imm8 ops, not just ADD), `find_addr_referenced_in_mov_instruction`→`find_riprel_mov_load_target_in_range`, and `elf_find_function_pointer`→`elf_find_function_ptr_slot`. Next: decide whether to also rename the OP4 string-xref builders (`elf_find_string_reference(s)`), or keep them as-is since their current names already match behavior.
- Session `RENAMES_OP2`: revisited `opco_patt` OP2 scan helpers and updated `metadata/backdoor_function_renames.json` with rename suggestions for the RIP-relative pointer/string scanners (`find_mov_lea_instruction`→`find_riprel_mov_or_lea`, `find_mov_instruction`→`find_riprel_mov`, `find_lea_instruction_with_mem_operand`→`find_riprel_lea`, `find_string_reference`→`find_string_lea_xref`) plus the displacement matcher (`find_lea_instruction`→`find_lea_with_displacement`) and the reg-only opcode finder (`find_reg2reg_instruction`→`find_reg_to_reg_instruction`). Next: do the same pass for OP3 (mem-operand sweep helpers) and decide whether any of the ELF-oriented string-xref helpers should be renamed for clarity.
- Session `RENAMES_OP1`: revisited `opco_patt` OP1 opcode helpers and updated `metadata/backdoor_function_renames.json` with two rename suggestions (`x86_dasm`→`x86_decode_instruction`, `find_function`→`find_function_bounds`) plus entries for the other OP1 helpers (`is_endbr64_instruction`, `find_function_prologue`, `find_call_instruction`). Next: repeat the pass for OP2/OP3 (`find_mov*`, `find_lea*`, mem-operand sweeps) and decide whether any of those deserve semantic renames beyond their current search-verb names.
...
- Session `CC5` revisit: clarified `secret_data_append_from_address`’s overloaded `addr` contract (NULL vs `(void *)1` toggles start-from-call while both reuse the caller RET; `addr >= 2` uses an explicit code pointer) and updated `secret_data_append_items`’ documentation to reflect the `ordinal == 0` stamp-and-skip path plus the 1-based callback index. Updated `metadata/functions_autodoc.json` and `docs/FUNCTION_PROGRESS.md`; validated with `./scripts/refresh_xzre_project.sh --check-only` and refreshed via `./scripts/refresh_xzre_project.sh` (rename report: `ghidra_scripts/generated/locals_rename_report.txt`, portable archive: `ghidra_projects/xzre_ghidra_portable.zip`). Next: decide whether `secret_data_item_t.ordinal` should be renamed/retagged as an enable flag and revisit `secret_data_append_item`’s “index 0 disables” note if it proves misleading.
