// /home/kali/xzre-ghidra/xzregh/107510_rsa_pubkey_sha256_fingerprint.c
// Function: rsa_pubkey_sha256_fingerprint @ 0x107510
// Calling convention: __stdcall
// Prototype: BOOL __stdcall rsa_pubkey_sha256_fingerprint(RSA * rsa, u8 * mdBuf, u64 mdBufSize, imported_funcs_t * funcs)


/*
 * AutoDoc: Grabs the exponent and modulus via RSA_get0_key, serialises the exponent first and the modulus second with bignum_mpint_serialize into a ~4 KiB stack buffer, and runs sha256_digest over the exact number of bytes produced. Any missing component or overflow of the 0x100a-byte scratch cancels the fingerprint.
 */

#include "xzre_types.h"

BOOL rsa_pubkey_sha256_fingerprint(RSA *rsa,u8 *mdBuf,u64 mdBufSize,imported_funcs_t *funcs)

{
  ...
  scratch_wipe_cursor = fingerprint_stream + 0x10;
  // AutoDoc: Pre-wipe the 0x100a-byte scratch arena. The loop clears the aligned tail and the explicit `fingerprint_stream[0..0xf]` stores below clear the head so no stack garbage contaminates the serialized fingerprint.
  ...
  if (((funcs != (imported_funcs_t *)0x0) && (rsa != (RSA *)0x0)) &&
    ...
    if ((rsa_exponent != (BIGNUM *)0x0) && (rsa_modulus != (BIGNUM *)0x0)) {
      ...
      success = bignum_mpint_serialize(fingerprint_stream,0x100a,&fingerprint_bytes,rsa_exponent,funcs);
      exp_serialized_len = fingerprint_bytes;
      if (((success != FALSE) &&
          ((fingerprint_bytes < 0x100a &&
           (success = bignum_mpint_serialize
                              // AutoDoc: Append the modulus immediately after the exponent and refuse any write that would exhaust the 0x100a-byte buffer.
                              (fingerprint_stream + fingerprint_bytes,0x100a - fingerprint_bytes,
        ...
        success = sha256_digest(fingerprint_stream,exp_serialized_len + fingerprint_bytes,mdBuf,mdBufSize,funcs);
        return success;
      }
    }
  }
