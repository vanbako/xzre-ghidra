// /home/kali/xzre-ghidra/xzregh/100C90_find_rel32_call_instruction.c
// Function: find_rel32_call_instruction @ 0x100C90
// Calling convention: __stdcall
// Prototype: BOOL __stdcall find_rel32_call_instruction(u8 * code_start, u8 * code_end, u8 * call_target, dasm_ctx_t * dctx)


/*
 * AutoDoc: Appends a secret-data breadcrumb, zeros the caller-supplied (or temporary) `dasm_ctx_t`, and walks `[code_start, code_end)` with `x86_decode_instruction`. Decode failures advance by one byte, while successes advance by `instruction_size` until the normalised CALL opcode (`0x168`) is seen. When `call_target` is non-NULL it further requires the rel32 destination (`instruction + instruction_size + imm_signed`) to match before returning TRUE. On success the context still describes the CALL so callers can immediately rewrite or inspect it.
 */

#include "xzre_types.h"

BOOL find_rel32_call_instruction(u8 *code_start,u8 *code_end,u8 *call_target,dasm_ctx_t *dctx)

{
  ...
  ctx_zero_stride = 0;
  decode_ok = secret_data_append_bits_from_addr_or_ret((void *)0x0,(secret_data_shift_cursor_t)0x81,4,7)
  ...
  if (decode_ok != FALSE) {
    ...
    while (code_start < code_end) {
      decode_ok = x86_decode_instruction(dctx,code_start,code_end);
      // AutoDoc: Decoder failed, so retry one byte later just like a pattern scan.
      if (decode_ok == FALSE) {
        code_start = code_start + 1;
      }
      else {
        if (((dctx->opcode_window).opcode_window_dword == X86_OPCODE_1B_CALL_REL32) &&
           ((call_target == (u8 *)0x0 ||
