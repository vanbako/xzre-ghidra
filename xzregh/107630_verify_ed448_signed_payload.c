// /home/kali/xzre-ghidra/xzregh/107630_verify_ed448_signed_payload.c
// Function: verify_ed448_signed_payload @ 0x107630
// Calling convention: __stdcall
// Prototype: BOOL __stdcall verify_ed448_signed_payload(sshkey * sshkey, u8 * signed_data, u64 sshkey_digest_offset, u64 signed_data_size, u8 * signature, u8 * ed448_raw_key, global_context_t * global_ctx)
...
BOOL verify_ed448_signed_payload
               (sshkey *sshkey,u8 *signed_data,u64 sshkey_digest_offset,u64 signed_data_size,
...

{
    ...
    if (status < 3) {
      if (status == 0) {
        ...
        success = dsa_pubkey_sha256_fingerprint
                          (sshkey->dsa,signed_data + sshkey_digest_offset,
  ...
  success = sha256_digest(serialized_key,serialized_key_len,signed_data + sshkey_digest_offset,
                        signed_data_size - sshkey_digest_offset,imports);
LAB_001076f8:
  if ((((success != FALSE) && (imports = global_ctx->imported_funcs, imports != (imported_funcs_t *)0x0)
       ) && (success = pointer_array_has_null(&imports->EVP_PKEY_new_raw_public_key,6), success == FALSE)
