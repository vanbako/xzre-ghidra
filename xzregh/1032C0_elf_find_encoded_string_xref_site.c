// /home/kali/xzre-ghidra/xzregh/1032C0_elf_find_encoded_string_xref_site.c
// Function: elf_find_encoded_string_xref_site @ 0x1032C0
// Calling convention: __stdcall
// Prototype: u8 * __stdcall elf_find_encoded_string_xref_site(elf_info_t * elf_info, EncodedStringId encoded_string_id, u8 * code_start, u8 * code_end)


/*
 * AutoDoc: Single-slot helper for the string catalogue. It emits a secret-data breadcrumb, then repeatedly calls `elf_find_encoded_string_in_rodata` (resuming from the last rodata pointer) until the requested `encoded_string_id` is encountered.
...
u8 * elf_find_encoded_string_xref_site
...

{
  ...
  telemetry_ok = secret_data_append_bits_from_call_site((secret_data_shift_cursor_t)0xd2,4,0xd,FALSE);
  if (telemetry_ok != FALSE) {
    ...
    while (string_cursor = elf_find_encoded_string_in_rodata(elf_info,&string_id_cursor,string_cursor),
          string_cursor != (char *)0x0) {
      // AutoDoc: Return the first LEA/MOV inside the requested code window that materialises this literal.
      xref_site = find_string_lea_xref(code_start,code_end,string_cursor);
      if (xref_site != (u8 *)0x0) {
        return xref_site;
      }
