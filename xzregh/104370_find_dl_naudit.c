...

{
  ...
  if (rtld_global_ro_sym != (Elf64_Sym *)0x0) {
    ...
    if (glro_lookup_string != (char *)0x0) {
      ...
      glro_string_xref = (u8 *)elf_get_code_segment(dynamic_linker_elf,&ldso_code_size);
      if ((glro_string_xref != (u8 *)0x0) &&
         ...
         (glro_string_xref = find_string_reference(glro_string_xref,glro_string_xref + ldso_code_size,glro_lookup_string), glro_string_xref != (u8 *)0x0)) {
        ...
        fake_allocator = get_lzma_allocator();
        ...
        while (mov_scan_cursor < glro_string_xref) {
          success = find_instruction_with_mem_operand_ex
                            (mov_scan_cursor,glro_string_xref,&insn_ctx,0x10b,(void *)0x0);
