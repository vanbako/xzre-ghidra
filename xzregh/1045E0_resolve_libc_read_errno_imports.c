// /home/kali/xzre-ghidra/xzregh/1045E0_resolve_libc_read_errno_imports.c
// Function: resolve_libc_read_errno_imports @ 0x1045E0
// Calling convention: __stdcall
// Prototype: BOOL __stdcall resolve_libc_read_errno_imports(link_map * libc, elf_info_t * libc_info, libc_imports_t * imports)


/*
 * AutoDoc: Treats the runtime libc `link_map` as another ELF image: run `elf_info_parse`, point the fake allocator at `libc_info`, and resolve `read` plus `__errno_location` through the bootstrap trampolines. Only when both slots land does the helper declare `libc_imports_t` ready so later socket helpers can avoid touching libc’s PLT.
 */

#include "xzre_types.h"

BOOL resolve_libc_read_errno_imports(link_map *libc,elf_info_t *libc_info,libc_imports_t *imports)
