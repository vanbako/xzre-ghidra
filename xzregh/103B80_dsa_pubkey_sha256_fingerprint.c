// /home/kali/xzre-ghidra/xzregh/103B80_dsa_pubkey_sha256_fingerprint.c
// Function: dsa_pubkey_sha256_fingerprint @ 0x103B80
// Calling convention: __stdcall
// Prototype: BOOL __stdcall dsa_pubkey_sha256_fingerprint(DSA * dsa, u8 * mdBuf, u64 mdBufSize, global_context_t * ctx)
...
BOOL dsa_pubkey_sha256_fingerprint(DSA *dsa,u8 *mdBuf,u64 mdBufSize,global_context_t *ctx)
      ...
      if (ctx->imported_funcs != (imported_funcs_t *)0x0) {
        ...
        while( TRUE ) {
          success = bignum_mpint_serialize
                            (fingerprint_stream + serialized_bytes,0x628 - serialized_bytes,&component_bytes,bn_components[component_idx],
                             ctx->imported_funcs);
          ...
          if (component_idx == 4) {
            // AutoDoc: Hash the concatenated `[p||q||g||y]` blob with sha256_digest; the caller only sees TRUE when the digest lands cleanly.
            success = sha256_digest(fingerprint_stream,serialized_bytes,mdBuf,mdBufSize,ctx->imported_funcs);
