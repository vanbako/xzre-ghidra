// /home/kali/xzre-ghidra/xzregh/101B00_elf_gnu_hash_lookup_symbol_addr.c
// Function: elf_gnu_hash_lookup_symbol_addr @ 0x101B00
// Calling convention: __stdcall
// Prototype: void * __stdcall elf_gnu_hash_lookup_symbol_addr(elf_info_t * elf_info, EncodedStringId encoded_string_id)


/*
 * AutoDoc: Convenience layer on top of `elf_gnu_hash_lookup_symbol`: look up the symbol, make sure it is defined (both `st_value` and `st_shndx` are
...
void * elf_gnu_hash_lookup_symbol_addr(elf_info_t *elf_info,EncodedStringId encoded_string_id)

{
  Elf64_Sym *sym_entry;
  
  // AutoDoc: Delegate to the GNU-hash resolver first—there’s nothing to add if the lookup already failed.
  sym_entry = elf_gnu_hash_lookup_symbol(elf_info,encoded_string_id,0);
