...
2. **Run the refresh loop.**
   ...
   - The script performs: metadata sync → headless import → `RenameFromLinkerMap.py` (driven by `metadata/linker_map.json`) → ApplySignatures/AutoDoc/TypeDocs/Locals → regeneration of `ghidra_scripts/generated/*` → export of `ghidra_projects/xzre_ghidra_portable.zip` → sync of new comments back into `xzregh/*.c`.
...
- Locals are harvested from both `xzre/xzre_code/` and the top-level `xzre/*.c` files. Missing functions are logged (expected when an object does not contain the symbol). Update `metadata/xzre_locals.json` when renaming locals/parameters so ApplyLocals can replay the new names.
